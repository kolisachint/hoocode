...
const agentDirEnvName = "HOOCODE_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "HOOCODE_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific HOOCODE_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force HOOCODE_OFFLINE=1 / HOOCODE_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.HOOCODE_OFFLINE = "1";
		env.HOOCODE_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
