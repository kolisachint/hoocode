...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("hoocode", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".hoocode/agent/sessions");
	...
	const globalAgentsMd = join(homedir(), ".hoocode/agent/AGENTS.md");
