...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@kolisachint/hoocode-agent";
...
```bash
npm install @kolisachint/hoocode-agent
```
...
```typescript
import { createAgentSession } from "@kolisachint/hoocode-agent";
...
The `Agent` class (from `@kolisachint/hoocode-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
import { createAgentSession, defineTool } from "@kolisachint/hoocode-agent";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@kolisachint/hoocode-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@kolisachint/hoocode-agent";
