...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@kolisachint/hoocode-tui";
...
```typescript
import { Container, type Focusable, Input } from "@kolisachint/hoocode-tui";
...
**In custom tools** via `hoo.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = hoo.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@kolisachint/hoocode-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@kolisachint/hoocode-tui";
```
...
```typescript
import { matchesKey, Key } from "@kolisachint/hoocode-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@kolisachint/hoocode-tui";
...
```typescript
import { getMarkdownTheme } from "@kolisachint/hoocode-agent";
import { Markdown } from "@kolisachint/hoocode-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@kolisachint/hoocode-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@kolisachint/hoocode-agent";
import { DynamicBorder } from "@kolisachint/hoocode-agent";
import { Container, type SelectItem, SelectList, Text } from "@kolisachint/hoocode-tui";
...
```typescript
import { BorderedLoader } from "@kolisachint/hoocode-agent";

hoo.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@kolisachint/hoocode-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@kolisachint/hoocode-agent";
import { Container, type SettingItem, SettingsList, Text } from "@kolisachint/hoocode-tui";

hoo.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
import { matchesKey, truncateToWidth } from "@kolisachint/hoocode-tui";
