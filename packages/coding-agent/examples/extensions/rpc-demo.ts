...
export default function (hoo: ExtensionAPI) {
	...
	hoo.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "hoocode RPC Demo (new session)" : "hoocode RPC Demo");
	...
	hoo.on("session_before_switch", async (event, ctx) => {
	...
	hoo.registerCommand("rpc-editor", {
	...
	hoo.registerCommand("rpc-prefill", {
