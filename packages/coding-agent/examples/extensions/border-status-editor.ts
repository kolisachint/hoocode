...
import type { Component, EditorTheme, TUI } from "@kolisachint/hoocode-tui";
...
export default function (hoo: ExtensionAPI) {
	...
	hoo.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await hoo.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = hoo.getThinkingLevel();
