 ...
 *
 * Usage:
 * - `hoocode --preset plan` - start with plan preset
...
import type { ExtensionAPI, ExtensionContext } from "@kolisachint/hoocode-agent";
import { DynamicBorder, getAgentDir } from "@kolisachint/hoocode-agent";
import type { Api, Model } from "@kolisachint/hoocode-ai";
import { Container, Key, type SelectItem, SelectList, Text } from "@kolisachint/hoocode-tui";
...
export default function presetExtension(hoo: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: hoo.getThinkingLevel(),
				tools: hoo.getActiveTools(),
			};
		}
		...
		if (preset.provider && preset.model) {
			...
			if (model) {
				const success = await hoo.setModel(model);
				if (!success) {
		...
		if (preset.thinkingLevel) {
			hoo.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = hoo.getAllTools().map((t) => t.name);
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await hoo.setModel(originalState.model);
				}
				hoo.setThinkingLevel(originalState.thinkingLevel);
				hoo.setActiveTools(originalState.tools);
			} else {
				hoo.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	hoo.registerShortcut(Key.ctrlShift("u"), {
	...
	hoo.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = hoo.getFlag("preset");
	...
	hoo.on("turn_start", async () => {
		if (activePresetName) {
			hoo.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
