...
export default function (hoo: ExtensionAPI) {
	hoo.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await hoo.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[hoocode] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await hoo.exec("git", ["add", "-A"]);
		const { code: commitCode } = await hoo.exec("git", ["commit", "-m", commitMessage]);
