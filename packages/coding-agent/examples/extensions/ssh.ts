 ...
 *
 * Usage:
 *   hoocode -e ./ssh.ts --ssh user@host
 *   hoocode -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (hoo: ExtensionAPI) {
	hoo.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	hoo.on("session_start", async (_event, ctx) => {
		...
		const arg = hoo.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
