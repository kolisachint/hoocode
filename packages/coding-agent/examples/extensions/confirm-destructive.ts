...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@kolisachint/hoocode-agent";

export default function (hoo: ExtensionAPI) {
	hoo.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
	...
	hoo.on("session_before_fork", async (event, ctx) => {
		if (!ctx.hasUI) return;
