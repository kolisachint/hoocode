 ...
 * working indicator shown while hoocode is streaming a response.
 *
 * Usage:
 *   hoocode --extension examples/extensions/working-indicator.ts
 *
 * Commands:
 ...
 *   /working-indicator reset     Restore hoocode's default spinner
 */

import type { ExtensionAPI, ExtensionContext, WorkingIndicatorOptions } from "@kolisachint/hoocode-agent";
...
function describeMode(mode: WorkingIndicatorMode): string {
	switch (mode) {
		...
		case "default":
			return "hoocode default spinner";
	}
}
...
export default function (hoo: ExtensionAPI) {
	...
	hoo.registerCommand("working-indicator", {
