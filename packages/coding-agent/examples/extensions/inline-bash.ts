 ...
 *
 * Start hoocode with this extension:
 *   hoocode -e ./examples/extensions/inline-bash.ts
 *
...
export default function (hoo: ExtensionAPI) {
	...
	hoo.on("input", async (event, ctx) => {
		...
		for (const { full, command } of matches) {
			try {
				const bashResult = await hoo.exec("bash", ["-c", command], {
					timeout: TIMEOUT_MS,
				});
