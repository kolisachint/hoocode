 ...
 *
 * Usage: hoocode --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@kolisachint/hoocode-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@kolisachint/hoocode-tui";
import { matchesKey, truncateToWidth, visibleWidth } from "@kolisachint/hoocode-tui";
...
export default function (hoo: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like a full-screen game extension would need)
	hoo.registerCommand("overlay-animation", {
	...
	hoo.registerCommand("overlay-anchors", {
	...
	hoo.registerCommand("overlay-margins", {
	...
	hoo.registerCommand("overlay-stack", {
	...
	hoo.registerCommand("overlay-overflow", {
	...
	hoo.registerCommand("overlay-edge", {
	...
	hoo.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	hoo.registerCommand("overlay-maxheight", {
	...
	hoo.registerCommand("overlay-sidepanel", {
	...
	hoo.registerCommand("overlay-toggle", {
	...
	hoo.registerCommand("overlay-passive", {
	...
	hoo.registerCommand("overlay-focus", {
	...
	hoo.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/hoocode/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/hoocode\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like a full-screen game extension
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(
			border("│") + padLine(th.fg("dim", " (a full-screen game extension uses same approach)")) + border("│"),
		);
		lines.push(border("│") + padLine(``) + border("│"));
