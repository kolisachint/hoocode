...
async function resolveGitHubRepo(hoo: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await hoo.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (hoo: ExtensionAPI): void {
	hoo.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(hoo, ctx.cwd);
		if (!resolvedRepo.ok) {
