 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit hoocode cleanly.
 */
...
export default function (hoo: ExtensionAPI) {
	// Register a /quit command that cleanly exits hoocode
	hoo.registerCommand("quit", {
		description: "Exit hoocode cleanly",
	...
	hoo.registerTool({
		...
		description: "Complete a task and exit hoocode",
	...
	hoo.registerTool({
		...
		description: "Deploy the application and exit hoocode",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await hoo.exec("npm", ["run", "deploy", params.environment], { signal });
