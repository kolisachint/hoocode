...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "@kolisachint/hoocode-agent";
import { StringEnum } from "@kolisachint/hoocode-ai";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@kolisachint/hoocode-tui";
...
export default function (hoo: ExtensionAPI) {
	hoo.on("session_start", async (_event, ctx) => reconstructState(ctx));
	hoo.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	hoo.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	hoo.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	hoo.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			hoo.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						hoo.appendEntry(SAVE_TYPE, getBoardDetails());
