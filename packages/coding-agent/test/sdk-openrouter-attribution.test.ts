...
describe("createAgentSession OpenRouter attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.HOOCODE_TELEMETRY;
		delete process.env.HOOCODE_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.HOOCODE_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://github.com/kolisachint/hoocode");
		expect(headers?.["X-OpenRouter-Title"]).toBe("hoocode");
