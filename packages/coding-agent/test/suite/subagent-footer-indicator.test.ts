 ...
 * the model-facing tool as `ExecuteTask`, but the interactive footer/resources wiring
 * checked `getActiveToolNames().includes("subagent")` and so never lit up.
 *
 * This test locks both ends together:
 *  - the ExecuteTask tool definition is named `ExecuteTask`,
...
describe("subagent footer indicator", () => {
	...
	it("ExecuteTask tool definition is named 'ExecuteTask'", () => {
		expect(createExecuteTaskToolDefinition().name).toBe("ExecuteTask");
	});

	it("lights up the footer and resources when the ExecuteTask tool is active", async () => {
		const harness = await createHarness({ tools: [taskTool] });
		harnesses.push(harness);

		expect(harness.session.getActiveToolNames()).toContain("ExecuteTask");

		const provider = new FooterDataProvider(harness.tempDir);
		// Mirror interactive-mode wiring.
		provider.setSubagentEnabled(harness.session.getActiveToolNames().includes("ExecuteTask"));
	...
	it("leaves the footer untouched when the ExecuteTask tool is absent", async () => {
		const harness = await createHarness();
		harnesses.push(harness);

		expect(harness.session.getActiveToolNames()).not.toContain("ExecuteTask");
