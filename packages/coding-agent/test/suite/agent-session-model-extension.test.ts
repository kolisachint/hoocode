...
import { fauxAssistantMessage, fauxToolCall, type Model } from "@kolisachint/hoocode-ai";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("setModel saves the model and emits model_select", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(hoo) => {
					hoo.on("model_select", async (event) => {
	...
	it("allows extension tool_call handlers to block tool execution", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(hoo) => {
					hoo.on("tool_call", async () => ({ block: true, reason: "Blocked by test" }));
				},
			],
		});
		harnesses.push(harness);
		...
		const harness = await createHarness({
			extensionFactories: [
				(hoo) => {
					...
					hoo.on("session_shutdown", async (event) => {
