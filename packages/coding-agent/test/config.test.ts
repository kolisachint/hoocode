...
const originalPiPackageDir = process.env.HOOCODE_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalPiPackageDir === undefined) {
		delete process.env.HOOCODE_PACKAGE_DIR;
	} else {
		process.env.HOOCODE_PACKAGE_DIR = originalPiPackageDir;
	}
	if (tempDir) {
...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const scopeDir = join(root, "@kolisachint");
	const packageDir = join(scopeDir, "hoocode-agent");
	mkdirSync(packageDir, { recursive: true });
	tempDir = prefix;
	process.env.HOOCODE_PACKAGE_DIR = packageDir;
...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	...
	const packageDir = join(root, "@kolisachint", "hoocode-agent");
	mkdirSync(packageDir, { recursive: true });
	...
	setExecPath(
		join(
			root,
			".pnpm",
			"@kolisachint+hoocode-agent@0.0.0",
			"node_modules",
			"@kolisachint",
			"hoocode-agent",
			"dist",
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	...
	const packageDir = join(globalDir, "node_modules", "@kolisachint", "hoocode-agent");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.HOOCODE_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@kolisachint", "hoocode-agent", "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@kolisachint+hoocode-agent@0.67.68\\node_modules\\@kolisachint\\hoocode-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@kolisachint/hoocode-agent")).toBe(
			"Run: pnpm install -g @kolisachint/hoocode-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@kolisachint/hoocode-agent")).toBeUndefined();
		expect(getUpdateInstruction("@kolisachint/hoocode-agent")).toBe(
			"Update @kolisachint/hoocode-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "@kolisachint/hoocode-agent"],
			display: `npm --prefix ${prefix} install -g @kolisachint/hoocode-agent`,
		});
	});
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		expect(command).toEqual({
			...
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@kolisachint/hoocode-agent"],
					display: `npm --prefix ${prefix} uninstall -g @kolisachint/hoocode-agent`,
				},
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", "@new-scope/pi"],
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@kolisachint/hoocode-agent", ["npm", "--prefix", prefix]);
	...
	test("self-update treats empty npmCommand as unset", () => {
		...
		expect(command?.args).toEqual(["--prefix", prefix, "install", "-g", "@kolisachint/hoocode-agent"]);
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(`npm --prefix "${prefix}" install -g @kolisachint/hoocode-agent`);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@kolisachint\\hoocode-agent";
		process.env.HOOCODE_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@kolisachint/hoocode-agent")).toBe("Run: npm install -g @kolisachint/hoocode-agent");
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "@kolisachint/hoocode-agent"],
			display: "bun install -g @kolisachint/hoocode-agent",
		});
	});
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			steps: [
				{
					command: "pnpm",
					args: ["remove", "-g", "@kolisachint/hoocode-agent"],
					display: "pnpm remove -g @kolisachint/hoocode-agent",
				},
				{
					command: "pnpm",
					args: ["install", "-g", "@new-scope/pi"],
	...
	test("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			steps: [
				{
					command: "yarn",
					args: ["global", "remove", "@kolisachint/hoocode-agent"],
					display: "yarn global remove @kolisachint/hoocode-agent",
				},
				{
					command: "yarn",
					args: ["global", "add", "@new-scope/pi"],
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			steps: [
				{
					command: "bun",
					args: ["uninstall", "-g", "@kolisachint/hoocode-agent"],
					display: "bun uninstall -g @kolisachint/hoocode-agent",
				},
				{
					command: "bun",
					args: ["install", "-g", "@new-scope/pi"],
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@kolisachint/hoocode-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@kolisachint/hoocode-agent")).toContain(
