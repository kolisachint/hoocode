...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		...
		delete process.env.HOOCODE_OFFLINE;
...
Content`,
		...
		it("should auto-discover root markdown skills from .hoo skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .hoo", async () => {
			const extDir = join(tempDir, ".hoocode", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".hoocode", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".hoocode", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".hoocode", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".hoocode", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".hoocode", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json hoo.extensions in extensions setting", async () => {
			// Create a package with hoo.extensions in package.json
			...
			// Should find the extensions declared in package.json hoo.extensions
	...
	describe("auto-discovered skill metadata", () => {
		...
		it("should use the project .hoo dir as baseDir for project .hoo skills", async () => {
			const projectBaseDir = join(tempDir, ".hoocode");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".hoocode", "agent");
		...
		it("should dedupe user skill entries when ~/.hoocode/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .hoo auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".hoo\n");

			const skillDir = join(tempDir, ".hoocode", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with hoo manifest", async () => {
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".hoocode", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .hoo settings base", () => {
			...
			const rel = relative(join(tempDir, ".hoocode"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in hoo manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json hoo.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".hoocode", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".hoocode", "npm")],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".hoocode", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".hoocode", "npm", "node_modules", "project-current");
			...
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				2,
				"npm",
				["install", "project-old@latest", "project-missing@latest", "--prefix", join(tempDir, ".hoocode", "npm")],
				undefined,
			);
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".hoocode", "npm", "node_modules", "example");
