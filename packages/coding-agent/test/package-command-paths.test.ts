...
describe("package commands", () => {
	...
	beforeEach(() => {
		...
		originalPiPackageDir = process.env.HOOCODE_PACKAGE_DIR;
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("hoocode install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "hoocode --help" or "hoocode install <source> [-l]".');
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: hoocode install <source> [-l]");
	...
	it("uses global npmCommand and current package name for forced self updates without checking the api", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@kolisachint", "hoocode-agent");
...
`,
		...
		process.env.HOOCODE_PACKAGE_DIR = selfPackageDir;
