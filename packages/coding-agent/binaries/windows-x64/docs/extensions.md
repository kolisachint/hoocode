...
| Package | Purpose |
|---------|---------|
| `@kolisachint/hoocode-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@kolisachint/hoocode-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@kolisachint/hoocode-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@kolisachint/hoocode-agent";
...
```typescript
import { isBashToolResult } from "@kolisachint/hoocode-agent";
...
```typescript
import { createLocalBashOperations } from "@kolisachint/hoocode-agent";
...
```typescript
import { SessionManager } from "@kolisachint/hoocode-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@kolisachint/hoocode-ai";
...
```typescript
import type { AutocompleteItem } from "@kolisachint/hoocode-tui";
...
```typescript
import { withFileMutationQueue } from "@kolisachint/hoocode-agent";
...
- [read.ts](https://github.com/kolisachint/hoocode/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@kolisachint/hoocode-agent";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/kolisachint/hoocode/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@kolisachint/hoocode-agent";
...
```typescript
import { Text, Component } from "@kolisachint/hoocode-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@kolisachint/hoocode-agent";
import { matchesKey } from "@kolisachint/hoocode-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@kolisachint/hoocode-agent";
