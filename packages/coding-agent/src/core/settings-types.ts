...
export interface Settings {
	...
	// default: "peek". How much of a tool call the transcript shows, least to
	// most: "radar" (one line per run of calls), "peek" (the call line and the
	...
	enableSemanticIndex?: boolean; // default: true - enable the semantic index layer for the always-on `SearchCodebase` tool: index the repo with the embsearch binary so it can fuse semantic hits. Set false to run lexical-only.
	...
	learnMinRequestRepeats?: number; // default: 3 - separate sessions a piece of work must be asked for before /learn proposes it as a slash command. Higher than the directive bar: a job that came up twice may just be a job that came up twice.
