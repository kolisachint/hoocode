# @kolisachint/hoocode-agent-core

Stateful agent with tool execution and event streaming. Built on `@kolisachint/hoocode-ai`.

## Installation

```bash
npm install @kolisachint/hoocode-agent-core
```
...
```typescript
import { Agent, streamProxy } from "@kolisachint/hoocode-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@kolisachint/hoocode-agent-core";
