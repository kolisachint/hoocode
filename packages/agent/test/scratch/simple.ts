...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".hoocode/skills")),
		source("user", join(homedir(), ".hoocode/agent/skills")),
...
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".hoocode/prompts")), source("user", join(homedir(), ".hoocode/agent/prompts"))],
